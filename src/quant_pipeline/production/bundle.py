...
def build_v3_analysis_bundle(*,run_root:Path,research:dict,source_manifest_hash:str,trial_coverage:dict)->Path:
    ...
    dossiers=list((run_root/"candidates").glob("*/dossier")); aggregate={"chronological_diagnostics":"chronology.parquet","resolution_comparison":"resolution_comparison.parquet","tail_ladder":"tail_ladder.parquet","horizon_ladder":"horizon_ladder.parquet"}
    for name,filename in aggregate.items(): _concat([d/filename for d in dossiers if (d/filename).exists()],out/f"{name}.parquet")
    empty_columns={"chronological_diagnostics":["candidate_id","month","n","mean_bps","evidence_role"],"resolution_comparison":["candidate_id","resolution","selected_cell","selected_state_bps","selected_interaction_lift_bps","selected_frequency","selected_n","surface_spread_bps","neighbor_effect_retention","plateau_area","evidence_role"],"tail_ladder":["candidate_id","tail_percent","n","contribution_bps","evidence_role"],"horizon_ladder":["candidate_id","target_id","horizon","n","mean_bps","evidence_role"]}
