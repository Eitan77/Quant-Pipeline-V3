...
def _aligned_stats(raw,direction):
    raw=np.asarray(raw,float); aligned=direction*raw; stats=distribution_stats(aligned); concentration=contribution_concentration(aligned); clean=raw[np.isfinite(raw)]
    return {"raw_mean_bps":float(clean.mean()) if len(clean) else np.nan,"raw_median_bps":float(np.median(clean)) if len(clean) else np.nan,"raw_positive_rate":float(np.mean(clean>0)) if len(clean) else np.nan,"direction_aligned_mean_bps":stats.get("mean_bps"),"direction_aligned_median_bps":stats.get("median_bps"),"favorable_rate":stats.get("win_rate"),"avg_favorable_bps":stats.get("avg_winner_bps"),"avg_adverse_bps":stats.get("avg_loser_bps"),**concentration}
...
def _concentration(raw,aligned,selected):
    base=pd.DataFrame({"raw":raw,"aligned":aligned,"security":selected.security_id.to_numpy(),"month":pd.to_datetime(selected.session_date).dt.to_period("M").astype(str),"session":selected.session_date.to_numpy()}); views={"baseline":base}
    if len(base):
        by_symbol=base.groupby("security").aligned.sum(); top=by_symbol.nlargest(5).index; by_month=base.groupby("month").aligned.sum(); by_session=base.groupby("session").aligned.sum(); cut=np.nanquantile(base.aligned,.99)
        views|={"remove_best_symbol":base[base.security!=by_symbol.idxmax()],"remove_top_5_symbols":base[~base.security.isin(top)],"remove_best_month":base[base.month!=by_month.idxmax()],"remove_largest_session":base[base.session!=by_session.abs().idxmax()],"remove_top_1pct_favorable_outcomes":base[base.aligned<cut],"winsorize_top_1pct_favorable_outcomes":base.assign(aligned=base.aligned.clip(upper=cut))}
    return [{"view":name,"remaining_n":len(part),"raw_mean_bps":float(part.raw.mean()) if len(part) else np.nan,"direction_aligned_mean_bps":float(part.aligned.mean()) if len(part) else np.nan,"evidence_role":"discovery_diagnostic"} for name,part in views.items()]
...
def build_candidate_dossiers(*,legacy_run,candidates:pd.DataFrame,candidate_summary:pd.DataFrame,duals:pd.DataFrame,specialist:pd.DataFrame):
    data=ProductionData(legacy_run); root=legacy_run.root/"candidates"; root.mkdir(exist_ok=True); masks={}; built=[]; security=pd.read_parquet(Path(legacy_run.config.project_root)/"reference/security_master.parquet",columns=["security_id","symbol"]).drop_duplicates("security_id")
    for candidate in candidates.to_dict("records"):
        ...
        eps,opps,timing,cross=_opportunities(obs,mask,ledger); pos=_positions(obs,opps); raw=y[pos]*1e4 if len(pos) else np.asarray([],float); aligned=direction*raw; selected=obs.iloc[pos].copy(); eligible=(a>=0)&(b>=0)&np.isfinite(y); sessions=int(obs.loc[eligible,"session_date"].nunique()); masks[candidate["candidate_id"]]=(grid,mask)
        ...
        chronology.extend({"candidate_id":candidate["candidate_id"],"diagnostic_type":"crossfit","period":str(x.fold),"active_n":int(x.heldout_n),"frequency":np.nan,"raw_mean_bps":float(x.heldout_state_bps),"direction_aligned_mean_bps":float(direction*x.heldout_state_bps),"independent_opportunities":np.nan,"evidence_role":"discovery_diagnostic"} for x in crossfit.itertuples()); _table(dossier/"chronology.parquet",chronology)
        counts=np.asarray(row["surface_counts"]); means=np.asarray(row["surface_means"]); lifts=np.asarray(row["incremental_surface"]); selected_cell=a_cell*resolution+b_cell; surface=[]
        for i in range(len(counts)):
            distance=abs(i//resolution-a_cell)+abs(i%resolution-b_cell); surface.append({"candidate_id":candidate["candidate_id"],"state_a":i//resolution,"state_b":i%resolution,"n":int(counts[i]),"mean_bps":float(means[i]*1e4) if np.isfinite(means[i]) else None,"interaction_lift_bps":float(lifts[i]*1e4) if np.isfinite(lifts[i]) else None,"is_selected":i==selected_cell,"is_direct_neighbor":distance==1,"manhattan_distance_from_selected":distance,"direction_aligned_mean_bps":float(direction*means[i]*1e4) if np.isfinite(means[i]) else None,"direction_aligned_interaction_lift_bps":float(direction*lifts[i]*1e4) if np.isfinite(lifts[i]) else None,"evidence_role":"discovery_diagnostic"})
        _table(dossier/"surface_cells.parquet",surface)
        comparison=duals[(duals.pair_id==row["pair_id"])&(duals.target_id==row["target_id"])]
        _table(dossier/"resolution_comparison.parquet",[{"candidate_id":candidate["candidate_id"],"resolution":int(x.v3_resolution),"selected_cell":int(x.selected_cell),"selected_state_bps":float(x.selected_state_bps),"selected_interaction_lift_bps":float(x.selected_interaction_lift_bps),"selected_frequency":float(x.selected_frequency),"selected_n":int(x.selected_n),"surface_spread_bps":float((x.best_cell_effect-x.worst_cell_effect)*1e4),"neighbor_effect_retention":float(x.neighbor_effect_retention),"plateau_area":int(x.plateau_area),"evidence_role":"discovery_diagnostic"} for x in comparison.itertuples()])
        ...
        for percent in (20,10,5,2,1):
            ...
            tails.append({"candidate_id":candidate["candidate_id"],"tail_percent":percent,"applicable":applicable,"reason":reason,"active_n":int(tail.sum()),"eligible_n":int(tail_eligible.sum()),"frequency":float(tail.sum()/max(1,tail_eligible.sum())),"raw_edge_bps":float(np.nanmean(y[tail])*1e4) if tail.any() else np.nan,"direction_aligned_edge_bps":float(direction*np.nanmean(y[tail])*1e4) if tail.any() else np.nan,"weighted_contribution_bps":float(direction*np.nanmean(y[tail])*1e4*tail.mean()) if tail.any() else np.nan,"episode_count":len(teps),"independent_opportunity_count":len(topps),"opportunities_per_day":len(topps)/max(1,int(obs.loc[tail_eligible,"session_date"].nunique())),"month_count":len(month_values),"favorable_month_fraction":float(np.mean(direction*np.asarray(month_values)>0)) if len(month_values) else np.nan,"raw_median_month_bps":float(np.median(month_values)) if len(month_values) else np.nan,"direction_aligned_median_month_bps":float(np.median(direction*np.asarray(month_values))) if len(month_values) else np.nan,"evidence_role":"discovery_diagnostic"})
        ...
        for spec in _compatible(data,target):
            ...
            horizons.append({**base,"n":int(np.isfinite(active_values).sum()),"raw_mean_bps":float(np.nanmean(active_values)) if np.isfinite(active_values).any() else np.nan,"direction_aligned_mean_bps":float(direction*np.nanmean(active_values)) if np.isfinite(active_values).any() else np.nan}); events.append({**base,"n":int(np.isfinite(event_values).sum()),"raw_mean_bps":float(np.nanmean(event_values)) if np.isfinite(event_values).any() else np.nan,"direction_aligned_mean_bps":float(direction*np.nanmean(event_values)) if np.isfinite(event_values).any() else np.nan})
        ...
        _table(dossier/"symbol_breakdown.parquet",symbols,columns=["candidate_id","security_id","n","raw_mean_bps","direction_aligned_mean_bps","evidence_role"]); sp=specialist[(specialist.pair_id==row["pair_id"])&(specialist.target_id==row["target_id"])&(specialist.resolution==resolution)].copy(); sp["candidate_id"]=candidate["candidate_id"]; sp["evidence_role"]="discovery_diagnostic"; _table(dossier/"specialist.parquet",sp); _table(dossier/"concentration.parquet",_concentration(raw,aligned,selected)); interaction=interaction_decomposition(target_bps=y*1e4,active_a=a==a_cell,active_b=b==b_cell); _table(dossier/"interaction.parquet",[{"candidate_id":candidate["candidate_id"],**interaction,"evidence_role":"discovery_diagnostic"}])
        ...
        sid=pd.factorize(selected.session_date,sort=True)[0] if len(selected) else np.asarray([],int); raw_inf=session_clustered_mean_se(raw,sid); aligned_inf=session_clustered_mean_se(aligned,sid); se=aligned_inf["session_clustered_se_bps"]
        _table(dossier/"inference.parquet",[{"candidate_id":candidate["candidate_id"],"session_n":raw_inf["session_n"],"raw_session_mean_bps":raw_inf["session_mean_bps"],"direction_aligned_session_mean_bps":aligned_inf["session_mean_bps"],"session_clustered_se_bps":se,"direction_aligned_t_stat":aligned_inf["session_mean_bps"]/se if np.isfinite(se) and se else np.nan,"episode_count":len(eps),"independent_opportunity_count":len(opps),"evidence_role":"discovery_diagnostic"}])
        lifecycle=[]; previous={}
        for ep in eps:
        ...
        built.append(candidate["candidate_id"]); (dossier/"dossier.json").write_text(json.dumps({"candidate_id":candidate["candidate_id"],"definition_hash":candidate["definition_hash"],"evidence_role":"discovery_diagnostic","files":sorted(x.name for x in dossier.iterdir())},indent=2,default=str),encoding="utf-8")
    for candidate_id,(grid,mask) in masks.items():
        peers=[{"candidate_id":other,"signal_jaccard":signal_jaccard(mask,other_mask)} for other,(other_grid,other_mask) in masks.items() if other!=candidate_id and other_grid==grid]; _table(root/candidate_id/"dossier"/"redundancy.parquet",peers,columns=["candidate_id","signal_jaccard"])
