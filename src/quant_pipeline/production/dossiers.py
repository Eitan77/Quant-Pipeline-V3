...
def build_candidate_dossiers(*,legacy_run,candidates:pd.DataFrame,candidate_summary:pd.DataFrame,duals:pd.DataFrame,specialist:pd.DataFrame):
    ...
    for candidate in candidates.to_dict("records"):
        ...
        eps,opps,timing,cross=_opportunities(obs,mask,ledger); pos=_positions(obs,opps); raw=y[pos]*1e4 if len(pos) else np.asarray([],float); aligned=direction*raw; selected=obs.iloc[pos].copy(); eligible=(a>=0)&(b>=0)&np.isfinite(y); sessions=int(obs.loc[eligible,"session_date"].nunique())
