...
def materialize_candidates(*,legacy_run,duals:pd.DataFrame,specialist:pd.DataFrame,research:dict,source_manifest_hash:str,candidate_rows:pd.DataFrame|None=None):
    policy=research.get("forensics",{}).get("candidate_policy",{}); pool=candidate_rows.copy() if candidate_rows is not None else materialization_pool(duals,min_active_n=int(policy.get("min_active_n",250)),min_abs_edge_bps=float(policy.get("min_abs_edge_bps",1.0)),top_k=int(policy.get("keep_top_k_per_target_resolution",250))); bundle=legacy_run.compile_registry(); features={x.feature_id:x for x in bundle.features}; targets={x.target_id:x for x in bundle.targets}; specialist_by={x.state_key:x for x in specialist.itertuples()} if "state_key" in specialist else {}; rows=[]; summaries=[]; now=datetime.now(timezone.utc).isoformat()
    if len(pool) and "state_key" not in pool:pool["state_key"]=[state_key(x) for x in pool.to_dict("records")]
    for row in pool.to_dict("records"):
        resolution=int(row["v3_resolution"]); cell=int(row["selected_cell"]); a_cell,b_cell=divmod(cell,resolution); threshold=float(policy.get("min_abs_edge_bps",1.0)); descriptive=row.get("direction_mode","auto")=="descriptive"; has_directional_edge=abs(row["selected_state_bps"])>=threshold; direction=0 if descriptive else int(np.sign(row["selected_state_return"])) if has_directional_edge else 0; candidate_type="descriptive" if descriptive else "directional" if has_directional_edge else "interaction_only"; reasons=[]
