...
def materialize_candidates(*,legacy_run,duals:pd.DataFrame,specialist:pd.DataFrame,research:dict,source_manifest_hash:str):
    ...
    for row in pool.to_dict("records"):
        resolution=int(row["v3_resolution"]); cell=int(row["selected_cell"]); a_cell,b_cell=divmod(cell,resolution); threshold=float(policy.get("min_abs_edge_bps",1.0)); has_directional_edge=abs(row["selected_state_bps"])>=threshold; direction=int(np.sign(row["selected_state_return"])) if has_directional_edge else 0; candidate_type="directional" if has_directional_edge else "interaction_only"; reasons=[]
