...
def specialist_probe(*,security_id,active,returns_bps,min_local_n=20):
    ...
    for s in np.unique(sec[mask]):
        local=mask&(sec==s); rows.append((s.item() if hasattr(s,"item") else s,int(local.sum()),float(y[local].mean())))
    eligible=[r for r in rows if r[1]>=min_local_n]
    if not eligible:return {"symbols_active":len(rows),"symbols_eligible":0,"fraction_positive":None,"fraction_negative":None,"effect_dispersion_bps":None,"top_symbol_share":None,"top5_symbol_share":None,"min_local_active_n":None,"median_local_active_n":None,"max_local_active_n":None}
